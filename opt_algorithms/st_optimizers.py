...
class ProjectedGradientStiefel(Optimizer):
    def __init__(self, params, lr=1.0, beta=0.5):
        ...
        super(ProjectedGradientStiefel, self).__init__(params, defaults)

        if len(self.param_groups) > 2:
            raise ValueError("ProjectedGradientStiefel doesn't support more than two per-parameter options "
                             "(parameter groups)")
        # for i in range(len(self.param_groups)):
        #     if len(self.param_groups[i]['params']) > 1:
        #         raise ValueError("ProjectedGradientStiefel doesn't support multiple parameters")
        ...
        for group in self.param_groups:
            ...
            for j in range(len(group['params'])):
                if stiefel:
                    ...
                    p1 = None
                    u, s, v = torch.empty((s, s), device=p.device), torch.empty((s,), device=p.device), torch.empty(
                        (N, s), device=p.device)
                    i = 0
                    while i < 1000:
                        ...
                        group['params'][j][:] = p1.t()[:]
                        loss1 = float(closure())
                        condition = loss1 <= loss + torch.trace(torch.mm(g.t(), (p1 - p))) + torch.trace(
                            torch.mm(p1 - p, (p1 - p).t())) / (2 * lr)
                        if condition:
                            break
                        else:
                            lr *= beta
                    ...
                    while i < 1000:
                        ...
                        condition = loss1 <= loss + trace(torch.matmul(g.t(), (p1 - p))) + trace(
                            torch.matmul(p1 - p, (p1 - p).t())) / (2 * lr)
                        if condition:
                            break
                        else:
                            lr *= beta
...
class ProjectedGradientLinear(Optimizer):
    def __init__(self, params, lr=1.0, beta=0.5):
        if lr < 0.0:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if beta <= 0.0 or beta >= 1.0:
            raise ValueError("Invalid beta: {}".format(lr))
        defaults = dict(lr=lr, beta=beta)
        super(ProjectedGradientLinear, self).__init__(params, defaults)
    ...
    @staticmethod
    def lin_project(x, r, out=None):
        # r is edge_index vector
        x_proj = torch.empty(x.shape, device=x.device)
        for i in range(x.shape[1]):
            xi = x[:, i]
            x_proj[:, i] = xi - r.t() @ xi / torch.pow(torch.norm(r, 2), 2) * r
            #
            # print('\n')
            # print(r.t() @ x)
            # print(r.t() @ x_proj)
            # print((torch.log(torch.abs(r.t() @ x)) - torch.log(torch.abs(r.t() @ x_proj)))/float(torch.log(torch.Tensor([10]))))
            # print(torch.norm(r))
        return x_proj
...
class LBFGS(Optimizer):
    ...
    .. warning::
        Right now all parameters have to be on adj single device. This will be
        improved in the future.

    .. note::
        This is adj very memory intensive optimizer (it requires additional
        ``param_bytes * (history_size + 1)`` bytes). If it doesn't fit in memory
        try reducing the history size, or use adj different algorithm.
    ...
    def step(self, closure):
        ...
        n_iter = 0
        # optimize for adj max of max_iter iterations
        while n_iter < max_iter:
            ...
            if line_search_fn is not None:
                ...
                if n_iter != max_iter:
                    ...
                    # the reason we do this: in adj stochastic setting,
